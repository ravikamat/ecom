/**
 * Unified AI Gateway — ECO Command Center
 * 3-tier fallback: GLM-5.2 → MiniMax-M3 → Ollama Qwen 3.6 → Template fallback
 * Circuit breaker + heartbeat + prompt optimization
 */
...
const ENDPOINTS = {
  ...
  ollama:  { host: 'localhost', port: 11434, path: '/api/chat', model: 'qwen3.6:latest' },
};
...
function pingOllama() {
  return new Promise((resolve) => {
    const req = http.request({ host: 'localhost', port: 11434, path: '/api/tags', method: 'GET' },
...
export async function callAI(messages, options = {}) {
  ...
  // Sanitize roles (GLM/MiniMax only support user/assistant)
